...
CREATE OR REPLACE TEMP VIEW events_pivot AS
  (WITH EVENTS_COUNT_BY_USER AS (SELECT USER_ID AS USER, EVENT_NAME, COUNT(*) AS EVENT_COUNT FROM EVENTS GROUP BY 1, 2)
  SELECT * FROM EVENTS_COUNT_BY_USER
  PIVOT (SUM(EVENT_COUNT) FOR EVENT_NAME IN ("cart", "pillows", "login", "main", "careers", "guest", "faq", "down", "warranty", "finalize", 
...
-- MAGIC %python
-- MAGIC display(spark.read.table('transactions').groupBy('user_id').count().where(col('count') > 1))
...
INNER JOIN EVENTS_PIVOT AS E ON E.USER = T.USER_ID
...
-- MAGIC %python
-- MAGIC # TODO
-- MAGIC df_events_pivot = spark.read.table('events_pivot')
-- MAGIC df_transactions = spark.read.table('transactions')
-- MAGIC (df_transactions
-- MAGIC     .join(df_events_pivot, df_transactions.user_id == df_events_pivot.user)
-- MAGIC     .createOrReplaceTempView("clickpath2"))
-- MAGIC display(spark.read.table('clickpath2'))
